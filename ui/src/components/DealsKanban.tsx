      ...
      {creating ? (
        <div className="border-b border-gray-200 bg-white px-4 py-4 sm:px-6">
          <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr_0.8fr_0.8fr_auto]">
            <input value={form.name} onChange={(e) => setForm((c) => ({ ...c, name: e.target.value }))} placeholder="Opportunity name" className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm" />
            <input value={form.accountName} onChange={(e) => setForm((c) => ({ ...c, accountName: e.target.value }))} placeholder="Account name" className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm" />
            <input value={form.amount} onChange={(e) => setForm((c) => ({ ...c, amount: e.target.value }))} placeholder="Amount" className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm" />
            <select value={form.stage} onChange={(e) => setForm((c) => ({ ...c, stage: e.target.value }))} className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm">
              <option value="QUALIFIED">Qualified</option>
              <option value="PROPOSAL">Proposal</option>
              <option value="NEGOTIATION">Negotiation</option>
              <option value="CLOSED_WON">Ready to Close</option>
            </select>
            <button onClick={handleCreate} disabled={saving || !form.name || !form.amount} className="rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:bg-gray-400">
