...
public class IntegrationConnectionController {
    ...
    @PreAuthorize("hasAuthority(T(com.dala.crm.security.CrmAuthorities).INTEGRATIONS_WRITE)")
    public IntegrationConnectionDto update(
            @PathVariable Long integrationId,
            @Valid @RequestBody IntegrationConnectionUpdateRequest request
    ) {
        return integrationConnectionService.update(integrationId, request);
    }

    @PostMapping("/{integrationId}/uninstall")
    @PreAuthorize("hasAuthority(T(com.dala.crm.security.CrmAuthorities).INTEGRATIONS_WRITE)")
    public IntegrationConnectionDto uninstall(@PathVariable Long integrationId) {
        return integrationConnectionService.uninstall(integrationId);
    }
